...
user_picture_container.shape = helpers.prrect(dpi(40), false, false, false, false)
user_picture_container.forced_height = dpi(180)
user_picture_container.forced_width = dpi(180)
...
local playground = require("ui.widgets.dashboard.playground")



local playergroundContainer = wibox.widget {
    playground,
    layout = wibox.layout.align.vertical,
    widget = wibox.container.background,
...
local calendar_box = create_boxed_widget(playergroundContainer, dpi(380), dpi(400), x.background)
...
local petal_top_left = create_url_petal("GH", x.color4, x.color12, "https://github.com/pyxodan/", true, true, false, true)
local petal_top_right = create_url_petal("YT", x.color1, x.color9, "https://youtube.com/", true, true, true, false)
local petal_bottom_right = create_url_petal("TGM", x.color2, x.color10, "https://elearning.tgm.ac.at/", false, true, true
    , true)
local petal_bottom_left = create_url_petal("OWA", x.color3, x.color11, "https://owa.tgm.ac.at/", true, false, true, true)
...
-- Left click - Take screenshot
    ...
    awful.button({}, 3, function()
        naughty.notify({ title = "Say cheese!", text = "Taking shot in 5 seconds", timeout = 4,
            icon = icons.getIcon("linex/screenshot_easy.png") })
