...
local function create_button(symbol, color, hover_color, cmd, key)
    ...
    if cmd then
        icon:buttons(gears.table.join(awful.button({}, 1, function()
            cmd()
        end), awful.button({}, 3, function() cmd() end)))
    end

    -- Add keybind to dict, if given
    if key then keybinds[key] = cmd end
...
local student = create_button("拾", x.color5, x.color13,
                              function()
...
local night_mode = create_button("望", x.color4, x.color12, apps.night_mode,
                                 "y")
local keys = create_button("", x.color2, x.color10, apps.passwords, "u")
local compositor = create_button("", x.color3, x.color11, apps.compositor,
                                 "i")
local restart_awesome = create_button("ﰇ", x.color1, x.color9,
                                      awesome.restart, "o")
local quit_awesome =
    create_button("", x.color5, x.color13, awesome.quit, "p")
...
local function create_stripe(widgets, bg)
    ...
    for _, widget in ipairs(widgets) do buttons:add(widget) end
...
screen.connect_signal("request::desktop_decoration", function(s)
    s.app_drawer = awful.popup {
        ...
        widget = {
            {
                -- Stripes
                create_stripe({terminal, editor, vim, browser, mail},
                              "#00000000"),
                create_stripe({files, office, okular, schedule, pyroWrite},
                              x.color8 .. "20"),
                create_stripe({resources, workspace, student, books, toDO},
                              x.color8 .. "40"),
                create_stripe({
                    night_mode, keys, compositor, restart_awesome, quit_awesome
                }, x.color8 .. "60"),
    ...
    s.app_drawer.bg = "#00000000"
    -- app_drawer.bg = beautiful.app_drawer_bg or x.background or "#111111"
    s.app_drawer.fg = beautiful.app_drawer_fg or x.foreground or "#FEFEFE"

    s.app_drawer:buttons(gears.table.join( -- Left click - Hide app_drawer
    awful.button({}, 1, function() app_drawer_hide(s) end),
    -- Right click - Hide app_drawer
    awful.button({}, 2, function() app_drawer_hide(s) end),
    -- Middle click - Hide app_drawer
    awful.button({}, 2, function() app_drawer_hide(s) end)))


    s.app_drawer_animation = rubato.timed {
        pos = s.geometry.height,
        intro = 0.2,
        outro = 0.2,
        duration = 0.4,
        easing = rubato.easing.quadratic,
            subscribed = function(pos) s.app_drawer.y = pos
        end
    }

    s.app_drawer_animation_timer = gears.timer {
        timeout = 0.4,
        single_shot = true,
        callback = function()
    ...
    s.app_drawer_grabber = nil
    s.app_drawer_animation_target = s.geometry.height
end)


function app_drawer_hide(s)
    awful.keygrabber.stop(s.app_drawer_grabber)
    s.app_drawer_animation_target = s.app_drawer_animation_target * (-1)
    s.app_drawer_animation.target = s.app_drawer_animation_target
    s.app_drawer_animation_timer:start()
end

function app_drawer_show(s)
    -- naughty.notify({text = "starting the keygrabber"})
    if(s.app_drawer.visible) then
        s.app_drawer_animation_timer:stop()

    end
    s.app_drawer_grabber = awful.keygrabber.run(
    ...
    s.app_drawer_animation.target = 0
end
