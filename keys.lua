...
awful.key({superkey, altkey}, "k", function()
    ...
    awful.spawn.with_shell("dmenu_run -p 'run:' -b -sb '" .. x.color13 .. "' -nb '" .. x.background .. "' -sf '" ..
                               x.background .. "'")
end, {
...
keys.clientbuttons = gears.table.join(awful.button({}, 1, function(c)
    ...
    awful.button({superkey}, 4, function(c)
        c.opacity = c.opacity + 0.1
    end), awful.button({superkey}, 5, function(c)
        c.opacity = c.opacity - 0.1
    end))
...
keys.tasklist_buttons = gears.table.join(awful.button({'Any'}, 1, function(c)
...
awful.button({'Any'}, 2, nil, function(c)
    c:kill()
end), awful.button({'Any'}, 3, function(c)
    c.minimized = true
